...
2. [ADR-0136](0136-workspace-owned-elastic-runtime.md) — one physical elastic
   Cube per Workspace with independently fenced concurrent Tool bindings.
3. [ADR-0135](0135-live-workspace-browser-and-lightweight-settlement.md) —
   lightweight Volume settlement and live, directory-scoped Workspace reads.
4. [ADR-0134](0134-user-managed-git-workspaces.md) — ordinary user-visible Git
   state with no platform Diff or post-Run commit/push.
5. [ADR-0133](0133-versioned-agent-identity-and-native-session-storage.md) —
   explicit Agent Revision routing and Runtime-native Session Storage.
6. [ADR-0132](0132-source-control-app-and-issue-automation.md) — GitLab/GitHub
   repository grants, Workspace-local Git authorization and user-directed Issue Runs.
7. [ADR-0131](0131-run-table-postgres-queue.md) — Run as the only PostgreSQL
   execution queue, typed Cancel/Steer control requests and terminal-only Outbox.
8. [ADR-0130](0130-user-managed-workspace-concurrency.md) — same-Session FIFO
   with user-managed concurrency across Sessions, terminals and shared files.
9. [ADR-0129](0129-multiplexed-worker-fact-connection.md) — one physical Fact
   connection per Worker with independently authorized logical Run Streams.
10. [ADR-0128](0128-kafka-soft-state-session-gateway.md) — Kafka AcceptedFact
   durability, rebuildable Gateway hot tails and cursor-free snapshot-first SSE.
11. [ADR-0127](0127-authority-gate-and-accepted-fact-bus.md) — one
...
12. [ADR-0124](0124-session-lease-fencing-authority.md) — one durable Session
   lease with a monotonically increasing fence across every Run effect boundary.
13. [ADR-0123](0123-isolated-preview-origins.md) — target-scoped capabilities
   and independent browser origins for untrusted application Preview.
14. [ADR-0120](0120-exclusive-full-vm-authority.md) — exclusive Cube full-VM
   state, restart adoption and the elastic/exclusive durability split.
15. [ADR-0118](0118-session-workspace-environment-and-ssh.md) — independent
   Session/Workspace/compute lifetimes, exclusive Cube Tool bindings and SSH access.
16. [ADR-0111](0111-current-production-architecture.md) — current end-to-end
   architecture, state authorities and scaling boundary.
17. [ADR-0112](0112-run-scoped-tool-capabilities.md) — Session grants, immutable
   Run Tool snapshots and Broker-side execution authorization.
18. [ADR-0113](0113-cloud-native-pi-subagents.md) — upstream-compatible Pi
   subagents on durable Child Sessions/Runs with explicit Workspace modes.
19. [ADR-0114](0114-conversation-subtree-delete-and-tail-prune.md) — recursive
   tree deletion and Pi-native conversation tail pruning.
20. [ADR-0115](0115-user-owned-development-environments.md) — tenant-aware,
   user-owned exclusive Cube development environments.
21. [ADR-0109](0109-postgres-session-reference-checkpoints.md) — PostgreSQL Pi
   SessionStorage as the sole conversation authority.
22. [ADR-0105](0105-pi-session-backend-conformance.md) — compatibility with Pi's
   public Session backend contract.
23. [ADR-0104](0104-human-session-tree-and-conversation-forks.md) — human tree
