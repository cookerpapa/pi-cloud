...
describe.sequential("CloudAgentRuntime", () => {
    ...
    async (cached) => {
      ...
      const runtime = new CloudAgentRuntime({
        ...
        commitCheckpoint: async (operation, sourceEvent) => {
          ...
          if ("commitCheckpoint" in execution) await execution.commitCheckpoint(operation);
          else {
      ...
      const runtime = new CloudAgentRuntime({
        ...
        streamFn: (_model, context) => {
          ...
          queueMicrotask(() => {
            request += 1;
            const message =
              request === 1 ? assistantError("network error") : assistant("recovered");
  ...
  it.each([false, true])(
    ...
    async (cached) => {
      ...
      expect(
        JSON.stringify(restored[0]?.messages).match(/visible-before-disconnect/g),
      ).toHaveLength(1);
