...
describe("Task reference under a Session lease", () => {
  it("round-trips the authority-issued identity at the minimum encoded length", () => {
    const token = createExecutionReference(LEASE_ID, ATTEMPT_ID, 1);
  ...
  it("rejects malformed and non-positive generations", () => {
    expect(() => createExecutionReference(LEASE_ID, ATTEMPT_ID, 0)).toThrow();
    expect(() => parseExecutionReference("pcer1_invalid")).toThrow();
  });

  it("is the only execution authority carried by an Agent event", () => {
    const executionReference = createExecutionReference(LEASE_ID, ATTEMPT_ID, 42);
