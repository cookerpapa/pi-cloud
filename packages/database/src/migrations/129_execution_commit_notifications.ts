...
/** A protocol cutover, not a transcript migration. Keep settled user history. */
export async function up(db: Kysely<unknown>): Promise<void> {
  await sql`do $$ begin
    if exists(select 1 from runs where state in ('claimed','provisioning','restoring','running','settling','cancel_requested'))
      ...
      raise exception 'Drain Runs and terminal Outbox before execution commit notification cutover';
    end if;
  end $$`.execute(db);
}

export async function down(): Promise<void> {
  throw new Error("Execution commit notification rollback requires a drained protocol deployment");
}
