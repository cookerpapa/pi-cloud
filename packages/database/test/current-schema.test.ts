...
describe("current PiCloud schema", () => {
  it("builds only the maintained authorities from an empty database", async () => {
    ...
    try {
      ...
      `.execute(database);
      expect(firstMigrationPass.rows).toHaveLength(148);
      expect(firstMigrationPass.rows[0]?.name).toBe("001_initial_control_plane");
      expect(firstMigrationPass.rows.at(-1)?.name).toBe("148_retire_tool_operation_ledger");
      ...
      expect(scopes.rows[0]?.table_type).toBe("VIEW");
      await runMigrations(database, "up");
      ...
      const applied = await sql<{ name: string }>`
        select name from kysely_migration order by name
      `.execute(database);
      expect(applied.rows.at(-1)?.name).toBe("148_retire_tool_operation_ledger");
      ...
      const sessionLogConstraint = await sql<{ definition: string }>`
        select pg_get_constraintdef(oid) as definition
          from pg_constraint
         where conname = 'pi_session_log_payload_valid'
      `.execute(database);
      expect(sessionLogConstraint.rows[0]?.definition).toContain("payload -> 'entry'");
